...
def my_height_field_to_mesh(func: Callable) -> Callable:
    ...
    def wrapper(difficulty: float, cfg: HfTerrainBaseCfg):
        ...
        result = func(difficulty, cfg)

        if isinstance(result, tuple):
            z_gen, origin_pixel = result
        else:
            z_gen = result
            origin_pixel = None
        ...
        if origin_pixel is None:
            ...
            origin = np.array([
                origin_pixel[0] * cfg.horizontal_scale,
                origin_pixel[1] * cfg.horizontal_scale,
                0.0,
            ])
            # px = border_pixels + round(origin[0] / cfg.horizontal_scale)
            # py = border_pixels + round(origin[1] / cfg.horizontal_scale)

            px = border_pixels + int(origin_pixel[0])
            py = border_pixels + int(origin_pixel[1])

            origin[2] = heights[px, py] * cfg.vertical_scale
